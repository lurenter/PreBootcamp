...
describe('Clase Gato', function() {
  ...
  xit('Debería llamar a la función constructor de Mamífero', function() {
  ...
  xit('Debería haber un Arreglo llamado descendencia y una propiedad nombre heredada de Mamífero', function() {
    expect(gato.descendencia).toEqual([]);
    expect(gato.nombre).toEqual('Capitán');
  });

  xit('Debería tener una propiedad color', function() {
    expect(gato.color).toEqual('Blanco');
  });
  
  xit('Toma su nombre en base al parametro pasado', function() {
  ...
  xit('Debería usar Object.create para heredar los métodos del Mamífero', function() {
    expect(typeof Gato.prototype.saludar).toEqual('function');
    expect(Gato.prototype.descendencia).toEqual(undefined);
  });

  xit('Deberia tener la propiedad constructor en el prototype que haga referencia a si mismo', function() {
  ...
  xit("Debería tener su propiedad nuevoHijo que acepte un color", function() {
  ...
  xit('El `nuevoHijo` de gato debería ser un Gato y no un Mamífero', function() {
    var JScat = gato.nuevoHijo('Amarillo');
    expect(JScat instanceof Gato).toEqual(true);
    expect(JScat.constructor).toEqual(Gato);
  });

  xit('Una instancia de Gato debería tener acceso a `Mamifero.prototype.saludar` en su prototype chain', function(){
