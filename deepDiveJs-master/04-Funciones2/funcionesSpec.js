...
describe('invocacionUnica', function() {
  ...
  xit('La función debería incrementar una única vez', function() {
...
describe('Contextos compartidos', function() {
  ...
  xit('Debería devolver un Objeto con cuatro métodos', function() {
  ...
  xit('Debería permitir a `incremento`, `incrementoPor10`, y `pedirValor` trabajar sobre el mismo valor', function() {
  ...
  xit('El método `cambiarValor` debería pisar el valor actual', function(){
    obj.incrementoPor10();
    obj.incremento();
...
describe('La lista secreta', function(){
  describe('Función `ListaDeFuncionesInvitados`', function() {
    ...
    xit('`ListaDeFuncionesInvitados` debería retornar un Arreglo', function() {
      expect(Array.isArray(funcionesInvitados)).toEqual(true);
    });

    xit('El largo de este Arreglo debería ser del mismo largo que el original', function() {
       expect(funcionesInvitados.length).toEqual(6);
    });

    xit('Cada indice del Arreglo debería ser una función', function(){
    ...
    xit('Las funciones en `funcionesInvitados` devuelve un invitado de `arregloInvitados` al pasarle el código', function(){
      var invitado = funcionesInvitados[0](codigoSecreto);

      expect(invitado).toEqual('Facu');
    });

    xit('Debería devolver el string `código secreto: invalido` si el código es incorrecto', function() {
      var invitado = funcionesInvitados[1](8910);

      expect(invitado).toEqual('código secreto: invalido');
    });

   xit('functions in the `guestNameFunction` do not have additional property or methods attached(use closure)', function(){
      var funcionTercerInvitado = funcionesInvitados[2];

      expect(Object.keys(funcionTercerInvitado).length).toEqual(0);
    });

    xit('`ListaDeFuncionesInvitados` no usa ningún método de Array.prototype', function(){
  ...
  describe('`armarListaDeInvitados`', function() {
    ...
    xit('`armarListaDeInvitados` debería ser una función', function() {
      expect(typeof armarListaDeInvitados === 'function').toEqual(true);
    });

    xit('`armarListaDeInvitados` debería devolver un Arreglo de strings', function() {
    ...
    xit('`listaOficial` debería tener la lista original de invitados', function() {
      var listaOficial = armarListaDeInvitados(funcionesInvitados, codigoSecreto);
